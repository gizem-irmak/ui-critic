...
function extractActionGroups(content: string, buttonLocalNames: Set<string>): ActionGroup[] {
  ...
  // Phase 1: Find opening tags of candidate containers
  // Matches: CardFooter, ButtonGroup, or any div/footer/section/nav with flex/grid/gap layout classes
  ...
  while ((openerMatch = openerRegex.exec(content)) !== null) {
    ...
    // For generic elements, require layout-related className tokens
    if (!isNamedContainer) {
    ...
    // Phase 2: Find matching closing tag using depth tracking
    const closingTag = tagName.toLowerCase() === 'cardfooter' ? 'CardFooter'
      : tagName.toLowerCase() === 'buttongroup' ? 'ButtonGroup' : tagName.toLowerCase();
    // Build a regex that matches opening or closing of the same tag
    ...
    if (containerEnd < 0) continue; // unmatched

    const containerContent = content.slice(openTagEnd, containerEnd);

    // Phase 3: Extract ALL descendant buttons (not just direct children)
    const buttons = extractButtonUsagesFromJsx(containerContent, buttonLocalNames, openTagEnd);

    console.log(`[U1.2] container candidate: <${tagName}> in content (offset ${openerMatch.index}), descendant CTAs = ${buttons.length}, labels = [${buttons.map(b => b.label).join(', ')}]`);
  ...
  // Deduplicate: if a smaller container is fully inside a larger one, keep only the smaller (more specific)
  const sorted = groups.sort((a, b) => a.offset - b.offset);
  const deduped: ActionGroup[] = [];
  for (const g of sorted) {
    const gEnd = g.offset + g.lineContext.length;
    // Check if any already-kept group fully contains this one
    ...
    if (!containedByExisting) {
      // Remove any existing groups that this one fully contains
    ...
    for (const group of actionGroups) {
      ...
      if (highs.length >= 2) {
        ...
        if (highStyleKeys.size === 1) {
          ...
          // +0.10 if buttons share same parent container (always true here — they're in an ActionGroup)
          u12Confidence += 0.10;
          // +0.10 if they share same high-emphasis styling (already verified by highStyleKeys.size === 1)
          u12Confidence += 0.10;
          // +0.05 if container uses horizontal layout (CardFooter or flex row)
          if (group.containerType === 'CardFooter' || /flex.*row|flex-row|gap-|space-x-/.test(group.lineContext)) {
            u12Confidence += 0.05;
          }
          // +0.05 if no semantic differentiation (no aria-describedby or role difference among buttons)
